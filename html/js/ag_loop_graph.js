...
function graphFromMap() {
  ...
  for (j = 0; j < level_h; j++) {
    for (i = 0; i < level_w; i++) {
      ...
      if (
        j + 1 < level_h &&
        (m[i][j + 1] === AG.B_BLOCK || m[i][j + 1] === AG.B_LADDER) &&
        (m[i][j] === AG.B_SPACE ||
          m[i][j] === AG.B_PRIZE ||
          m[i][j] === AG.B_BIBPRIZE || // m[i][j] === AG.B_LADDER ||
          m[i][j] === AG.B_ONEUP ||
          m[i][j] === AG.B_KEY ||
          m[i][j] === AG.B_INITIAL_GOAL ||
          m[i][j] === AG.B_GOAL ||
          m[i][j] === AG.B_MARKER ||
          m[i][j] === AG.B_MONSTER)
      ) {
      ...
      if (
        ...
        if (
          m[i][j + 1] !== AG.B_LADDER &&
          !isInList(JSON.stringify(graphNode(i, j)), string_drop)
        ) {
  ...
  for (z = 0; z < len; z++) {
    ...
    if (
      j.y + 2 < level_h &&
      isLikeSpace(m[j.x][j.y + 1]) &&
      m[j.x][j.y + 2] === AG.B_BLOCK
    ) {
  ...
  while (z < len) {
    ...
    if (
      isInList(JSON.stringify(graphNode(stop.x, stop.y + 1)), string_ladder)
    ) {
  ...
  while (z < len) {
    ...
    if (
      (z + 1 < floor.length && //&& start.x !== floor[z+1].x
