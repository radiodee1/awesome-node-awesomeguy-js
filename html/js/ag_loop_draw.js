...
function setTileMapData() {
  tiles_a = copyArraysExpand_tileset(
    "tiles1",
    AG.TILEMAP_WIDTH,
    AG.TILEMAP_HEIGHT
  );
  tiles_b = copyArraysExpand_tileset(
    "tiles2",
    AG.TILEMAP_WIDTH,
    AG.TILEMAP_HEIGHT
  );
  tiles_c = copyArraysExpand_tileset(
    "tiles3",
    AG.TILEMAP_WIDTH,
    AG.TILEMAP_HEIGHT
  );
  tiles_d = copyArraysExpand_tileset(
    "tiles4",
    AG.TILEMAP_WIDTH,
    AG.TILEMAP_HEIGHT
  );
...
function setMonsterData() {
  monster_a = copyArraysExpand_tileset(
    "monster_r0",
    AG.MONSTER_WIDTH,
    AG.MONSTER_HEIGHT
  );
  monster_b = copyArraysExpand_tileset(
    "monster_r1",
    AG.MONSTER_WIDTH,
    AG.MONSTER_HEIGHT
  );
  monster_c = copyArraysExpand_tileset(
    "monster_l0",
    AG.MONSTER_WIDTH,
    AG.MONSTER_HEIGHT
  );
  monster_d = copyArraysExpand_tileset(
    "monster_l1",
    AG.MONSTER_WIDTH,
    AG.MONSTER_HEIGHT
  );
  monster_ud1 = copyArraysExpand_tileset(
    "monster_ud1",
    AG.MONSTER_WIDTH,
    AG.MONSTER_HEIGHT
  );
  monster_ud2 = copyArraysExpand_tileset(
    "monster_ud2",
    AG.MONSTER_WIDTH,
    AG.MONSTER_HEIGHT
  );
...
function cutTile(tileset, tile_ignore, num) {
  ...
  var z = ctx.getImageData(
    Math.floor(l * AG.TILE_WIDTH),
    Math.floor(k * AG.TILE_WIDTH),
    8,
    8
  );
...
function drawScoreWords() {
  ...
  if (guy.y > 16) {
    ...
    for (i = 0; i < 6; i++) {
      ...
      drawTile_8(
        square,
        (scorePos + i) * AG.TILE_WIDTH + scrollx,
        1 * AG.TILE_HEIGHT + scrolly,
        scrollx,
        scrolly,
        PAINT_TRANSPARENT,
        number_alpha
    ...
    for (i = 0; i < 6; i++) {
      ...
      drawTile_8(
        square,
        (livesPos + i) * AG.TILE_WIDTH + scrollx,
        1 * AG.TILE_HEIGHT + scrolly,
        scrollx,
        scrolly,
        PAINT_TRANSPARENT,
...
function drawScoreNumbers(pos, num, p) {
  ...
  for (i = 0; i < p; i++) {
    ...
    if (showZeros === 1 || placesValue !== 0) {
      ...
      drawTile_8(
        square,
        (pos + i - p + c) * AG.TILE_WIDTH + scrollx,
        1 * AG.TILE_HEIGHT + scrolly,
        scrollx,
        scrolly,
        PAINT_TRANSPARENT,
...
function drawMonsters() {
  ...
  if (monster_num > 0) {
    for (i = monster_offset; i < monster_num; i++) {
      markerTest = false;
      is_up_down = false;
      if (sprite[i].type !== "monster" && sprite[i].type !== "super_monster")
        continue;

      if (sprite[i].active === true && sprite[i].move === 0) {
        // && ! preferences_graph_control ) {
        ...
        if (sprite[i].facingRight === true) {
          ...
          if (
            xx + 3 >= level_w - 3 ||
            typeof map_objects[xx + 2] === "undefined"
          ) {
            xx = level_w - 3;
            //sprite[i].x -= 8 * 2;
            continue;
      ...
      if (sprite[i].move !== 0) {
        ...
        if (
          ...
          (map_objects[xx][yy + 1] === AG.B_BLOCK ||
            map_objects[xx][yy + 2] === AG.B_BLOCK ||
            map_objects[xx][yy + 1] === AG.B_LADDER ||
            map_objects[xx - 1][yy + 1] === AG.B_BLOCK)
        ) {
        ...
        if (
          sprite[i].move === AG.UP &&
          yy + 3 < level_h && // map_objects[xx][yy] === AG.B_SPACE   ||
          (map_objects[xx][yy + 1] === AG.B_BLOCK ||
            map_objects[xx][yy + 2] === AG.B_BLOCK ||
      ...
      if (
        yy + 2 < level_h &&
        (sprite[i].move !== AG.UP || sprite[i].move === AG.DOWN) &&
        false && // sprite[i].move !==  AG.RIGHT && sprite[i].move !==  AG.LEFT &&
        map_objects[xx][yy + 1] !== AG.B_BLOCK &&
        map_objects[xx][yy] !== AG.B_LADDER &&
        map_objects[xx][yy + 1] !== AG.B_LADDER &&
...
function drawLevel(unused) {
  ...
  for (j = baseX - 1; j < baseX + tilesWidthMeasurement + 3; j++) {
    //32
    for (i = baseY - 1; i < baseY + tilesHeightMeasurement + 3; i++) {
      //24
...
function initLevel() {
  ...
  for (i = 0; i < level_h; i++) {
    ...
    for (j = 0; j < level_w; j++) {
      ...
      //add monster here
      if (map_objects[j][i] === AG.B_MONSTER) {
        //32,64
  ...
  for (i = 0; i < level_h; i++) {
    ...
    for (j = 0; j < level_w; j++) {
      //x
      if (map_objects[j][i] === AG.B_PLATFORM) {
        //32,64
  ...
  $.ajax({
    ...
    success: function (xml) {
      ...
      for (level_in = 1; level_in <= 10; level_in++) {
        $(xml)
          .find('game level[number="' + level_in + '"]')
          .each(function () {
            ...
            var tiles_level = $(this)
              .find("tiles_level")
              .text()
              .replace(" ", ""); // a
            var tiles_objects = $(this)
              .find("tiles_objects")
              .text()
              .replace(" ", ""); // b
